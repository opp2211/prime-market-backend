...
create or replace function generate_public_code(prefix text)
    returns text
    language plpgsql
as $$
declare
    alphabet constant text := '23456789ABCDEFGHJKLMNPQRSTUVWXYZ';
    result text := upper(prefix) || '-';
    i int;
begin
    for i in 1..7 loop
...
(
    ...
    public_code     varchar(16)    not null default generate_public_code('TX'),
...
create unique index if not exists ux_user_account_txs_public_code
    on user_account_txs (public_code);
...
begin
    if tg_op = 'UPDATE' then
        if new.id <> old.id
            ...
            raise exception 'Updating id, public_code, user_account_id, or amount is not allowed for user_account_txs';
        end if;
