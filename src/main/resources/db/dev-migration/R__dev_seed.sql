...
from games g
         join (values
                   ...
                   -- Albion Online
                   ('albion-online', 'server', 'Server', 10)
) as v(game_slug, slug, title, sort_order)
              on lower(g.slug) = lower(v.game_slug)
where not exists(
    select 1
...
from context_dimensions cd
         ...
         join (values
                   ...
                   ('albion-online', 'server', 'asia', 'Asia', 30)
) as v(game_slug, dimension_slug, value_slug, value_title, sort_order)
              on lower(g.slug) = lower(v.game_slug)
...
insert into category_context_dimensions (
    category_id,
...
from categories c
    ...
    values
        ...
        ('albion-online', 'services', 'server', 10, 'europe')
) as v(game_slug, category_slug, dimension_slug, sort_order, default_value_slug)
              on lower(g.slug) = lower(v.game_slug)
...
from categories c
    ...
    values
        ...
        -- Albion Online / services
        ('albion-online', 'services', 'service-type', 'Service Type', 'select', true, 10)
) as v(game_slug, category_slug, slug, title, data_type, is_required, sort_order)
              on lower(g.slug) = lower(v.game_slug)
                  and lower(c.slug) = lower(v.category_slug)
where not exists (
    select 1
...
from category_attributes ca
    ...
    values
        ...
        ('albion-online', 'services', 'service-type', 'leveling', 'Leveling', 20)
) as v(game_slug, category_slug, attribute_slug, option_slug, option_title, sort_order)
              on lower(g.slug) = lower(v.game_slug)
                  and lower(c.slug) = lower(v.category_slug)
...
from categories c
    ...
    values
        ...
        ('albion-online', 'services', 'account-share', 'Account Share', 20)
) as v(game_slug, category_slug, slug, title, sort_order)
              on lower(g.slug) = lower(v.game_slug)
                  and lower(c.slug) = lower(v.category_slug)
where not exists (
    select 1
...
insert into category_trade_field_configs (
    category_id,
    field_slug,
    is_visible,
    is_required,
...
from categories c
    ...
    values
        ...
        ('albion-online', 'services', 'delivery-methods', true, false, true, null, null, 60)
) as v(
       game_slug,
       category_slug,
       field_slug,
